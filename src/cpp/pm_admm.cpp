...
int main(int argc, char **argv) {
    ...
    try {
        ...
        std::vector<int> raw = config["track_idx"].as<std::vector<int>>();                   // 3*ntrack
        m.track_idx.reserve(3 * raw.size());
        for (int v : raw)
            for (int j = 0; j < 3; ++j) m.track_idx.push_back(3 * v + j);
        m.track_targ = yaml_mat("track_targ", config, raw.size(), 3).reshaped<Eigen::RowMajor>(m.track_idx.size() * 3, 1);
        // std::cout << m.track_targ;
    ...
    leap::examples::MatchProblem probObj(urdf, gravity, q0, v0, x0_r, v0_r, q0, vf,
                                                          m, ry.rp, pins, T, deg, trackPen);
...

    {
        ...
        if (auto rc = probObj.ropeCost_) {
            rc->dumpRope("sol_kinova_rope_rope.csv");
            std::cout << "Value of cost: " << rc->value() << std::endl;
        }
