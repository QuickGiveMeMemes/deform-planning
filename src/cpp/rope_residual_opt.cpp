...
inline RopeYaml loadRopeYaml(const std::string& path) {
  ...
  if (N < 3) throw std::runtime_error("rope yaml: need >= 3 vertices");
  rp.rest_pos.resize(N, 3);
  for (int i = 0; i < N; ++i) {
    if (vs[i].size() != 3) throw std::runtime_error("rope yaml: vertex must be [x,y,z]");
    for (int j = 0; j < 3; ++j) rp.rest_pos(i, j) = vs[i][j].as<double>();
  }
  ...
  if (mn["mass_uniform"].as<bool>()) {
    ...
    for (int i = 0; i < N; ++i) mv(i) = a[i].as<double>();
  }
  rp.mass.resize(3 * N);
  for (int i = 0; i < N; ++i) rp.mass.segment<3>(3 * i).setConstant(mv(i));
  ...
  if (dn["damping_uniform"].as<bool>()) {
    ...
    for (int i = 0; i < N; ++i) dv(i) = a[i].as<double>();
  }
  out.damping.resize(3 * N);
  for (int i = 0; i < N; ++i) out.damping.segment<3>(3 * i).setConstant(dv(i));


  // --- stretching (N-1 edges) ---
//   rp.enable_stretch = y["stretching"]["enable"].as<bool>();
  rp.kstretch = Eigen::VectorXd::Constant(
      N - 1, y["stretching"]["stiffness"].as<double>() );

//   // --- bending (N-2 interior) ---
//   rp.enable_bending = y["bending"]["enable_bending"].as<bool>();
  rp.kbend = Eigen::VectorXd::Constant(
      N - 2,  y["bending"]["k_bend"].as<double>());

  // --- pinned vertices ---
  rp.pinned_idx.clear();
  for (const auto& p : y["control"]["pinned"]) {
    const int v = p.as<int>();
    if (v < 0 || v >= N) throw std::runtime_error("rope yaml: pinned index out of range");
    rp.pinned_idx.push_back(v);
  }
  if (rp.pinned_idx.empty()) throw std::runtime_error("rope yaml: no pinned vertices");
  return out;
}
...
Eigen::VectorXd parseVec(const std::string& s, int n, const char* what) {
  ...
  while (std::getline(ss, tok, ',')) {
    bool blank = true;
    for (char c : tok) blank = blank && std::isspace(static_cast<unsigned char>(c));
    if (!blank) vals.push_back(std::stod(tok));
  }
  ...
  Eigen::VectorXd v(n);
  for (int i = 0; i < n; ++i) v(i) = vals[static_cast<size_t>(i)];
  return v;
}

Eigen::VectorXd vecArg(int argc, char** argv, const char* key, int n,
                       const std::string& dflt) {
  const std::string s = args(argc, argv, key, dflt);
  if (s.empty()) return Eigen::VectorXd::Zero(n);
...
int main(int argc, char** argv) {
  ...
  const std::string urdf = args(argc, argv, "urdf", "assets/kinova3_1arm.urdf");
  const std::string ropeYaml = args(argc, argv, "rope", "assets/arm_rope_1pin.yaml");
  const std::string pinFrame = args(argc, argv, "pinframe", "arm1_end_effector_link");
  const int deg = static_cast<int>(argd(argc, argv, "deg", 16));
  const double T = argd(argc, argv, "T", 3.0);
  const bool limits = argd(argc, argv, "limits", 0.0) != 0.0;  // nu==0: no torque box
  const bool gn = argd(argc, argv, "gn", 1.0) != 0.0;
  const bool pfill = argd(argc, argv, "pfill", 1.0) != 0.0;
  const double muMax = argd(argc, argv, "mumax", 1e7);
  const double mu0 = argd(argc, argv, "mu0", 10.0);
  const double mugrow = argd(argc, argv, "mugrow", 3.0);
  const double tol = argd(argc, argv, "tol", 1e-6);
  const double dsSeg = argd(argc, argv, "dsseg", 5e-2);
  const double sMax = argd(argc, argv, "smax", 3.0);
  const int stallFlow = static_cast<int>(argd(argc, argv, "stall", 6));
  const double stallRel = argd(argc, argv, "stallrel", 1e-2);
  const int interNodeReport = static_cast<int>(argd(argc, argv, "internode", 16));
  ...
  try {
    ...
    if (nFree <= 0) throw std::runtime_error("rope yaml: every vertex is pinned");

    // One frame per pinned vertex, in yaml order. Single-pin config here; for
    // multi-pin, take a comma-separated pinframe list and zip with pinned_idx.
    if (rp.pinned_idx.size() != 1)
      throw std::runtime_error("kinova_rope: pinframe= supports exactly one pin; "
                               "extend to a frame list for multi-pin ropes");
    pins.push_back({rp.pinned_idx[0], pinFrame});
    ...
    const leap::RobotModel probe = leap::RobotModel::fromUrdf(
        leap::examples::armConfig(urdf, {pinFrame}, gravity));
    nq = probe.nq();
    nv = probe.nv();
    q0 = vecArg(argc, argv, "q0", nq, "");
    v0 = vecArg(argc, argv, "v0", nv, "");
    qf = vecArg(argc, argv, "qf", nq, "1.0,0.5,1.0,1.0,1.0,0.5,1.0");
    vf = vecArg(argc, argv, "vf", nv, "");
    ...
    if (pinErr > 1e-6)
      std::fprintf(stderr, "kinova_rope: WARNING pinned vertex %d is %.3e m from "
                   "frame '%s' at q0\n", rp.pinned_idx[0], pinErr, pinFrame.c_str());
  } catch (const std::exception& e) {
    std::fprintf(stderr, "kinova_rope: %s\n", e.what());
    return 2;
  }

  leap::examples::ContinuousRopeResidualProblem probObj(
      urdf, gravity, q0, v0, x0_r, v0_r, qf, vf, xf_r, ry.rp, pins, T, deg);
  ...
  std::printf("[kinova_rope|alm] urdf=%s  rope=%s\n"
              ...
              "  rope: n=%d free=%d pin=v%d@%s  ks=%.3g  bend=%s(kb=%.3g)  damping=%s\n"
              "  deg=%d T=%.2f  gn=ON  mumax=%.3g  backend=%s\n",
              urdf.c_str(), ropeYaml.c_str(), nq, nv, nq + 3 * nFree, nRope, nFree,
              ry.rp.pinned_idx[0], pinFrame.c_str(), ry.rp.kstretch(0),
              "ON",
              ry.rp.kbend.size() ? ry.rp.kbend(0) : 0.0,
              "UNMODELED", deg, T, muMax, leap::backendName(backend));
