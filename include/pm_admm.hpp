...
// ADMM select point matching (as opposed to full shape matching)
// #include <algorithm>
...
namespace leap::examples {
    ...
    struct MatchSpec {
        // Idx-based tracking to final pos
        std::vector<int> track_idx;
        Eigen::VectorXd track_targ;  // (ntrack * 3), stacked/flattened for convenience
    };

    struct PointMatchCost final : public RolloutCostBase {
        struct PinCache {
        ...
        PinCache pin_;
        Eigen::VectorXd x0_, v0_;
        MatchSpec m;

        // Due to structural limitations FK must be performed internally
        PointMatchCost(pinocchio::Model &m, const ADMMParams &rp, std::vector<PinSpec> pins,
                       std::string name, Eigen::MatrixXd x0, Eigen::MatrixXd v0, MatchSpec target,
                       double k)
            : rope_(rp), model_(m), nq_(m.nq), nv_(m.nv), name_(name), data_(m), k(k), m(target) {
        ...
        void dumpRope(const std::string &path) const {
            ...
            for (size_t i = 0; i < pin_.vertex.size(); ++i)
                f << (i ? ";" : "") << pin_.vertex[i];
            f << "\n";
            for (int t = 0; t < T; ++t) {
                for (int j = 0; j < N3; ++j)
        ...
        void fillCache(const Eigen::Ref<const Eigen::MatrixXd> &Q, double T) override {
            ...
            // Smooth interp 0 -> 1
            // std::cout << m.track_idx.size() << '\n';
            for (int t = 0; t < rope_.r.T; ++t) {
                for (size_t j = 0; j < m.track_idx.size(); ++j) {
                    const int d = m.track_idx[j];                       // = 3*vertex + component
                    loss_grad_per_step(t, d) = w(t) * (pin_.xhist_(t, d) - m.track_targ(j));
                    // std::cout << '(' << t << ',' << d << ',' << m.track_targ(j) << ')' << '\n';
                }
                // std::cout << std::endl;
            }         
            // std::fprintf(stderr, "N3=%d  xhist=%ld  grad=%ld \n",
            //  N3, pin_.xhist_.cols(), rope_.grad().cols());
            ...
            // std::cout << value() << '\n';
            // std::cout << "Value of cost: " << value() << std::endl;
        }

        double w(int t) const {
            return (t == rope_.r.T - 1) ? k : std::clamp(k / 10.0, 1.0, 100.0);
        }
        ...
        double value() const override {
            ...
            for (int t = 0; t < rope_.r.T; ++t) {
                double e2 = 0.0;
                for (size_t j = 0; j < m.track_idx.size(); ++j) {
                    const double e = pin_.xhist_(t, m.track_idx[j]) - m.track_targ(j);
                    e2 += e * e;
                }
                s += 0.5 * w(t) * e2;
            }
    ...
    struct SimpleArmControlCost final : public CostBase {
        SimpleArmControlCost(const RobotModel &m)
            : nq_arm_(m.nq()), nv_arm_(m.nv()), name_("arm") {}
    ...
    //     FramePinConstraint(const RobotModel &m, std::vector<PinSpec> pins, Eigen::Matrix3Xd tgt,
    //                        std::string name = "frame_pin")
    ...
    class MatchProblem final : public TrajectoryProblem {
      public:
        MatchProblem(const std::string &urdfPath, const Eigen::Vector3d &gravity,
                    ...
                    MatchSpec match,// Eigen::MatrixXd xf_r, // Eigen::MatrixXd vf_r,
            ...
            ropeCost_ =
                std::make_shared<PointMatchCost>(loc_m, rp, pins, "admm rope", x0_r, v0_r, match, k);
            ...
            //     {std::make_shared<FramePinConstraint>(m, pins, goal, "frame_pin_f"), W::LastNode});

            // Boundary conditions
            ms.attachments.push_back(
                {std::make_shared<PinConstraint>(indexRange(d.offQ(), nq_arm), q0_arm, "pin_q0_arm",
                                                 Enforce::Dirichlet),
                 W::FirstNode});

            ms.attachments.push_back(
                {std::make_shared<PinConstraint>(indexRange(d.offV(), nv_arm), v0_arm, "pin_v0_arm",
                                                 Enforce::Dirichlet),
                 W::FirstNode});

            // qf, vf only for ref traj generation
            // ms.attachments.push_back({std::make_shared<PinConstraint>(indexRange(d.offQ(),
            // nq_arm),
            //                                                           qf_arm, "pin_qf_arm"),
            //                           W::LastNode});
        ...
        std::shared_ptr<PointMatchCost> ropeCost_;
      ...
      private:
        ...
        void buildInitialGuess(const Eigen::VectorXd &q0, const Eigen::VectorXd &qf,
                               const Eigen::VectorXd &v0, const Eigen::VectorXd &vf) {
