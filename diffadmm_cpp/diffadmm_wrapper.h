...
namespace diffadmm_wrapper
{
    ...
    using HostLR = Kokkos::View<typename View::data_type, Kokkos::LayoutRight,
                                Kokkos::HostSpace, Kokkos::MemoryTraits<Kokkos::Unmanaged>>;

    template <typename View>
    View np_to_kokkos(const NpArray &a, const std::string &name)
    {
        ...
        HostLR<View> h_np = alloc<HostLR<View>>(buf.ptr, buf.shape.data(), std::make_index_sequence<R>{});

        auto mirror = Kokkos::create_mirror_view(device); // device layout (LayoutLeft expected), host space
        Kokkos::deep_copy(mirror, h_np);                  // converts into device layout
    ...
    template <typename View, std::size_t... I>
    HostLR<View> kokkos_wrap(typename View::value_type *ptr, const View &v, std::index_sequence<I...>)
    {
        return HostLR<View>(ptr, v.extent(I)...);
    }

    template <typename View>
    py::array_t<typename View::value_type> kokkos_to_np(const View &a)
    {
        using valtype = typename View::value_type;

        auto mirror = Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), a);
        constexpr std::size_t R = View::rank();

        auto size = kokkos_shape(a, std::make_index_sequence<R>{});
        py::array_t<valtype> np_a(size);
