...
namespace diffadmm_wrapper
{
    ...
    namespace
    {
        ...
        struct jxu_in
        {
            ...
            Kokkos::View<double **> A_inv;
            Kokkos::View<double ***> penaltyDt_stretch, D_bend, W_bend;
            Kokkos::View<double **> rest_curv, mass, L0, stiffness_stretch, penalty_stretch, stiffness_bend;
            Kokkos::View<bool *> is_pinned;
        };

        // ---------- Input Processing ----------

        fwd_in build_fwd_in(const NpArray &x0, const NpArray &v0, const NpArray &mass,
                            ...
                            const std::optional<NpArray> &pin_positions, int T)
        {
            ...
            for (int &p : pin_indices) // normalize negative indices
                if (p < 0)
                    p += N;
                         ...
                         bool bending_as_force, double admm_tol, int admm_check_interval)
        {
            ...
            auto A_inv = build_A_inv(in.mass, 1.0 / (dt * dt), in.penalty_stretch,
                                     in.is_pinned, penalty_bend, bending_admm,
                                     stretching_admm, bending_as_force, in.stiffness_bend);
            ...
            admm_forward(in.x0, in.v0, in.mass, in.L0, in.stiffness_stretch,
                         in.penalty_stretch, in.pin_pos, in.is_pinned, A_inv, dt,
                         1.0 / (dt * dt), in.damping, ADMM_ITERS, T, x_hist, y_hist,
                         z_hist, dual_hist, z_bend_hist, dual_bend_hist, in.stiffness_bend,
                         penalty_bend, bending_admm, stretching_admm, bending_as_force,
                         in.rest_curv, admm_tol, admm_check_interval);
            Kokkos::fence("forward_fence");

            return fwd_out{x_hist, y_hist, z_hist, dual_hist, z_bend_hist, dual_bend_hist,
                                              ...
                                              int gmres_m, int gmres_restart, double gmres_tol)
        {
            ...
            admm_control_dense_state(
                ...
                gmres_m, gmres_restart, gmres_tol, J_xu);
        ...
        int admm_check_interval, int t, int gmres_m, int gmres_restart, double gmres_tol)
    {
        ...
        // Hand the already-resident device views straight through; no re-conversion.
        jxu_in jin;
